 ...
 */
const getSampleToLocationUrl = ({id = null, sample_id = null, location_id = null} = {}) =>
 ...
 */
export async function getSampleToLocations(sample_id) {
  return fetch(getSampleToLocationUrl({sample_id}), {
    method: 'GET',
  })
    .then(response => response.json())
    .then(json => json.map(_json => parseSampleToLocation(_json)));
...
/**
 * Gets samples to location related to the location ID.
 *
 * @param {string} location_id - Specific location ID.
 * @returns {Promise<Object[]>} Related samples to location.
 */
export async function getSamplesToLocation(location_id) {
  return fetch(getSampleToLocationUrl({location_id}), {
    method: 'GET',
  })
    .then(response => response.json())
    .then(json => json.map(_json => parseSampleToLocation(_json)));
...
/**
 * Gets sample to location related to the sample to location ID, or the sample ID and location ID.
 * @param {string} sample_id - Specific sample ID.
 * @param {string} location_id - Specific location ID.
 * @returns {Promise<Object>} Related sample to location.
 */
export async function getSampleToLocation(sample_id, location_id) {
  return fetch(getSampleToLocationUrl({sample_id, location_id}), {
    method: 'GET',
  })
    .then(response => response.json())
    .then(json => json.map(_json => parseSampleToLocation(_json)));
 ...
 */
export const formatSampleToLocation = sampleToLocation => {
