import {API_KEY, BASE_URL, SEQUENCE_LENGTH} from '../data/config';
import {sortSequences} from '../helpers';
...
export async function putSample(sample) {
  return fetch(getSampleUrl({id: sample.id}), {
    method: 'PUT',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
...
export const parseSample = sample => ({
  ...
  ratings: sample?.ratings ?? null,
  datetime: sample?.datetime ?? new Date().toISOString(),
});
 ...
 */
export const formatSample = sample => {
  let outgoingSample = {
    api_key: API_KEY,
    name: sample.name,
